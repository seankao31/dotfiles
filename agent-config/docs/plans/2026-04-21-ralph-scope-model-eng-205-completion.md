...
The new design doc (`agent-config/docs/specs/2026-04-21-ralph-scope-model-design.md`, committed as `aad03e0`) is not modified by this plan. Its abstract "implementation ticket" references remain intentional — the doc is a durable design artifact and cementing ENG-215's ID inside it adds brittleness without buying clarity (the linked Linear comment on ENG-205 already names ENG-215 explicitly).

**Out of scope for this plan (intentional):**
- `agent-config/docs/playbooks/ralph-v2-usage.md` — its single-project language describes *current* behavior, which doesn't change until ENG-215 ships. ENG-215 owns the playbook update.
...
## Task 1: Update v2 spec forward-reference (ENG-203 → ENG-205 design + ENG-215 implementation)

**Files:**
- Modify: `agent-config/docs/specs/2026-04-17-ralph-loop-v2-design.md:469`

Context: The v2 spec's Contract Summary section item 3 notes the v2 single-project scope limit and points at ENG-203 as the future fix. With ENG-205's design subsuming ENG-203 (now canceled) and ENG-215 filed as the implementation ticket, that pointer is stale.
...
git commit -m "ralph-v2 spec: update multi-project forward-reference to ENG-205 design + ENG-215 impl (ENG-205)"
```
...
- Anything that would bias ENG-215's implementation in a wrong direction
...
- Linear ENG-215 filing — already done (before this plan).
- ENG-203 cancellation — already done (before this plan).

...
4. **Move the issue to "In Progress"** with `linear issue update ID --state "In Progress"` — but read the current state first via `linear issue view ID --json` and branch on it:
   ...
   - `In Design`: stop and ask. The state signals that an interactive design session was started but didn't conclude. Starting implementation without first concluding the design (→ `Approved`) means moving forward without a finalized spec. Prompt the user to choose: resume `/ralph-spec` to finish the design, manually mark the issue `Approved` and then return here to proceed, or manually reset it to `Todo`/`Backlog` to abandon. Do not transition the issue yourself — all state changes out of `In Design` belong to `/ralph-spec` or the user. See the `In Design` state subsection for context.
